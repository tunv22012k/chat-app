...
const filteredUsersList = computed(() => {
  return props.usersOnline.filter(row => row.name.toLowerCase().includes(searchQuery.value.toLowerCase()))
})
</script>
...
<template>
  <div class="card mb-sm-3 mb-md-0 contacts_card">
    <div class="card-header">
      <h3 class="d-flex text-white">Online<span class="badge bg-success ms-2">{{ usersOnline.length }}</span></h3>
      ...
      <div class="contacts">
        <li v-for="user in filteredUsersList" :key="user.id" @click="$emit('selectReceiver', user)">
          ...
          <div class="d-flex bd-highlight">
            <div class="img_cont">
              <img :src="user.id === $root.user.id ? '/images/current_user.jpg' : '/images/other_user.jpg'"
